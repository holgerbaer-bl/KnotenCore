...
## AOT Module Linking & The Standard Library (StdLib)

KnotenCore supports splitting scripts into modules via the `import "module.nod";` keyword. The `Compiler` natively processes `import` statements at compile-time by:
1. Lexing and Parsing the child file relative to the caller.
2. Checking against the `imported_files` hashset to prevent circular dependencies.
3. Automatically injecting the generated Bytecode into the active AOT stream.
