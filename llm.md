...
- **Security & Sandboxing**: KnotenCore is a fully sandboxed engine. Security issues (FFI bypasses, file system escapes) are critical. Autonomous agents MUST review `SECURITY.md` and `CONTRIBUTING.md` when proposing PRs or debugging OS-level bugs.
- **Execution Backend (AOT/JIT)**: The engine is migrating intensive logic towards flat bytecode processing emitting `OpCode` to the `VM`. The `VM` dispatcher (`machine.rs`) operates as a rigorous Stack Machine running an Arithmetic Logic Unit (ALU). Postfix evaluation is strict. The VM natively supports variables (`OpSetGlobal`/`OpGetGlobal`) and OS interfaces (`OpReadFile`, `OpExternCall`). `OpExternCall` resolves unregistered AST identifiers and natively vectors directly through `crate::natives::bridge::BridgeModule` handling dynamically inferred prefixes ("ui_", "registry_", "fs_"). Control Flow is fully supported via `Compiler Backpatching` inside `compiler.rs` (`Jump`/`JumpIfFalse` placeholders are emitted and subsequently overwritten with computed offsets).
