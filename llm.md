...
- **Execution Backend (AOT/JIT)**: The engine is migrating intensive logic towards flat bytecode processing emitting `OpCode` to the `VM`. The `VM` dispatcher (`machine.rs`) operates as a rigorous Stack Machine running an Arithmetic Logic Unit (ALU). Postfix evaluation is strict. The VM natively supports variables via a `globals` mapping structure using `OpSetGlobal`/`OpGetGlobal`. The VM interacts with files through `OpReadFile`, demanding explicit `AgentPermissions` checks and path sanitization. It evaluates arrays through `str_split`/`arr_contains`. Control Flow is fully supported via `Compiler Backpatching` inside `compiler.rs` (`Jump`/`JumpIfFalse` placeholders are emitted and subsequently overwritten with computed offsets).
