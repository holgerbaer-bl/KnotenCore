...
*   **📦 WGPU Texture Atlas & Rodio Audio (Milestone 13)**: WGPU Texture Atlas parsing and Amiga-style Polyphonic Audio natively supported via `rodio` handling dynamic sample pitch offsets.
...
*   **🌍 Gravity & Biomes (Milestone 18)**: Transforming the engine into a playable game. Introducing real-time player physics with AABB collision, gravity, and jumping. The world generation now supports diverse biomes (Water, Sand, Grass, Stone) and procedural flora like trees, all controlled via the AST.
*   **✨ The Beauty Update (Milestone 19)**: Massive visual overhaul. Integrated a high-quality RPG texture atlas, implemented **Directional Lighting** (Sunlight), and added **Distance Fog** for a seamless horizon. Upgraded UV mapping to support multi-textured blocks (e.g., Grass tops vs Dirt sides).
...
*   **🌍 Gravitation & Biome (Meilenstein 18)**: Verwandelt die Engine in ein echtes Spiel. Einführung von Echtzeit-Physik mit AABB-Kollision, Schwerkraft und Sprungmechanik. Die Weltgenerierung unterstützt nun verschiedene Biome (Wasser, Sand, Gras, Stein) und prozedurale Flora wie Bäume, direkt gesteuert über den AST.
*   **✨ The Beauty Update (Meilenstein 19)**: Massive optische Überarbeitung. Integration eines RPG-Textur-Atlas, Implementierung von **Directional Lighting** (Sonnenlicht) und **Distance Fog** für einen weichen Horizont. Erweitertes UV-Mapping unterstützt nun unterschiedliche Texturen pro Würfelseite (z. B. Gras oben, Erde an den Seiten).
