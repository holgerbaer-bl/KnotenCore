...
impl ExecutionEngine {
    ...
    pub fn evaluate_inner(&mut self, node: &Node) -> ExecResult {
        match node {
            ...
            Node::Identifier(name) => {
                if let Some(v) = self.get_var(name) {
                    ...
                    ExecResult::Fault {
                        ...
                        node: "Node::Identifier".into(),
                    }
                }
            }
            Node::Assign(name, expr) => match self.evaluate_inner(expr) {
                ExecResult::Value(v) => {
            ...
            Node::Sin(expr) => match self.evaluate_inner(expr) {
                ExecResult::Value(RelType::Float(v)) => ExecResult::Value(RelType::Float(v.sin())),
                ExecResult::Value(_) => ExecResult::Fault {
                    msg: "Sin expects float".into(),
                    node: "Node::Sin".into(),
                },
                err => err,
            },
            Node::Cos(expr) => match self.evaluate_inner(expr) {
                ExecResult::Value(RelType::Float(v)) => ExecResult::Value(RelType::Float(v.cos())),
                ExecResult::Value(_) => ExecResult::Fault {
                    msg: "Cos expects float".into(),
                    node: "Node::Cos".into(),
                },
                err => err,
            },
            ...
            Node::And(l, r) => match (self.evaluate_inner(l), self.evaluate_inner(r)) {
                (ExecResult::Value(RelType::Bool(a)), ExecResult::Value(RelType::Bool(b))) => {
                    ExecResult::Value(RelType::Bool(a && b))
                }
                _ => ExecResult::Fault {
                    msg: "And expects two booleans".into(),
                    node: "Node::And".into(),
                },
            },
            Node::Or(l, r) => match (self.evaluate_inner(l), self.evaluate_inner(r)) {
                (ExecResult::Value(RelType::Bool(a)), ExecResult::Value(RelType::Bool(b))) => {
                    ExecResult::Value(RelType::Bool(a || b))
                }
                _ => ExecResult::Fault {
                    msg: "Or expects two booleans".into(),
                    node: "Node::Or".into(),
                },
            },
            Node::Not(expr) => match self.evaluate_inner(expr) {
                ExecResult::Value(RelType::Bool(v)) => ExecResult::Value(RelType::Bool(!v)),
                _ => ExecResult::Fault {
                    msg: "Not expects a boolean".into(),
                    node: "Node::Not".into(),
                },
            },
            Node::Time | Node::GlobalTime => {
                ExecResult::Value(RelType::Float(self.startup_time.elapsed().as_secs_f64()))
            }
            Node::Mat4Mul(l, r) => {
                let lv = match self.evaluate_inner(l) {
                    ExecResult::Value(RelType::Array(v)) => v,
                    _ => {
                        return ExecResult::Fault {
                            msg: "Mat4Mul expects array".into(),
                            node: "Node::Mat4Mul".into(),
                        };
                    }
                };
                let rv = match self.evaluate_inner(r) {
                    ExecResult::Value(RelType::Array(v)) => v,
                    _ => {
                        return ExecResult::Fault {
                            msg: "Mat4Mul expects array".into(),
                ...
                if lv.len() != 16 || rv.len() != 16 {
                    return ExecResult::Fault {
                        msg: "Mat4Mul expects 16-element arrays".into(),
                        node: "Node::Mat4Mul".into(),
                    };
                }
                ...
                ExecResult::Value(RelType::Array(
                    res.into_iter().map(|f| RelType::Float(f as f64)).collect(),
                ))
            }
            ...
            Node::ArrayGet(arr, idx) => {
                let a = match self.evaluate_inner(arr) {
                    ExecResult::Value(RelType::Array(v)) => v,
                    _ => {
                        return ExecResult::Fault {
                            msg: "Target is not an array".into(),
                            node: "Node::ArrayGet".into(),
                        };
                    }
                };
                let i = match self.evaluate_inner(idx) {
                    ExecResult::Value(RelType::Int(v)) => v as usize,
                    _ => {
                        return ExecResult::Fault {
                            msg: "Index is not an integer".into(),
                            node: "Node::ArrayGet".into(),
                        };
                    }
                };
                if i < a.len() {
                    ExecResult::Value(a[i].clone())
                } else {
                    ExecResult::Fault {
            ...
            Node::ArraySet(arr_expr, idx_expr, val_expr) => {
                let val = match self.evaluate_inner(val_expr) {
                    ExecResult::Value(v) => v,
                    err => return err,
                };
                if let Node::Identifier(name) = &**arr_expr {
                    let i = match self.evaluate_inner(idx_expr) {
                        ExecResult::Value(RelType::Int(v)) => v as usize,
                        _ => {
                            return ExecResult::Fault {
                                msg: "Index is not an integer".into(),
                    ...
                    ExecResult::Fault {
                        msg: "ArraySet only supported on identifiers currently".into(),
                        node: "Node::ArraySet".into(),
                    }
                }
            }
            Node::ArrayPush(arr_expr, val_expr) => {
                ...
                if let Node::Identifier(name) = &**arr_expr {
                    ...
                    ExecResult::Fault {
                        msg: "ArrayPush only supported on identifiers currently".into(),
            ...
            Node::MapGet(map_expr, key_expr) => {
                let m = match self.evaluate_inner(map_expr) {
                    ExecResult::Value(RelType::Object(v)) => v,
                    _ => {
                        return ExecResult::Fault {
                            msg: "Target is not a map/object".into(),
                            node: "Node::MapGet".into(),
                        };
                    }
                };
                let k = match self.evaluate_inner(key_expr) {
                    ExecResult::Value(RelType::Str(v)) => v,
                    _ => {
                        return ExecResult::Fault {
                            msg: "Key is not a string".into(),
                            node: "Node::MapGet".into(),
                        };
                    }
                };
            ...
            Node::MapSet(map_expr, key_expr, val_expr) => {
                ...
                if let Node::Identifier(name) = &**map_expr {
                    ...
                    ExecResult::Fault {
                        msg: "MapSet only supported on identifiers currently".into(),
                        node: "Node::MapSet".into(),
                    }
                }
            }
            ...
            Node::PropertyGet(obj_expr, prop) => {
                let o = match self.evaluate_inner(obj_expr) {
                    ExecResult::Value(RelType::Object(v)) => v,
                    _ => {
                        return ExecResult::Fault {
                            msg: "Target is not an object".into(),
                            node: "Node::PropertyGet".into(),
                        };
                    }
                };
            ...
            Node::PropertySet(obj_expr, prop, val_expr) => {
                ...
                if let Node::Identifier(name) = &**obj_expr {
                    ...
                    ExecResult::Fault {
                        msg: "PropertySet only supported on identifiers currently".into(),
                        node: "Node::PropertySet".into(),
                    }
                }
            }
            Node::Index(container, idx) => {
                let c = match self.evaluate_inner(container) {
                    ExecResult::Value(v) => v,
                    err => return err,
                };
                let i = match self.evaluate_inner(idx) {
                ...
                match (c, i) {
                    (RelType::Array(a), RelType::Int(idx)) => {
                        if (idx as usize) < a.len() {
                            ExecResult::Value(a[idx as usize].clone())
                        } else {
                            ExecResult::Fault {
                    ...
                    (RelType::Str(s), RelType::Int(idx)) => {
                        if let Some(ch) = s.chars().nth(idx as usize) {
                            ...
                            ExecResult::Fault {
                                msg: "String index out of bounds".into(),
                                node: "Node::Index".into(),
                            }
                        }
                    }
                    _ => ExecResult::Fault {
                        msg: "Invalid index operation".into(),
                        node: "Node::Index".into(),
                    },
                }
            }
            Node::Concat(l, r) => {
                ...
                match (lv, rv) {
                    ...
                    (RelType::Array(mut a), RelType::Array(b)) => {
                        a.extend(b);
                        ExecResult::Value(RelType::Array(a))
                    }
                    _ => ExecResult::Fault {
                        msg: "Concat expects strings or arrays".into(),
                        node: "Node::Concat".into(),
                    },
                }
            }

            // Bitwise
            Node::BitAnd(l, r) => match (self.evaluate_inner(l), self.evaluate_inner(r)) {
                (ExecResult::Value(RelType::Int(a)), ExecResult::Value(RelType::Int(b))) => {
                    ExecResult::Value(RelType::Int(a & b))
                }
                _ => ExecResult::Fault {
                    msg: "Bitwise AND expects integers".into(),
                    node: "Node::BitAnd".into(),
                },
            },
            Node::BitShiftLeft(l, r) => match (self.evaluate_inner(l), self.evaluate_inner(r)) {
                (ExecResult::Value(RelType::Int(a)), ExecResult::Value(RelType::Int(b))) => {
                    ExecResult::Value(RelType::Int(a << b))
                }
                _ => ExecResult::Fault {
                    msg: "Bitwise SHL expects integers".into(),
                    node: "Node::BitShiftLeft".into(),
                },
            },
            Node::BitShiftRight(l, r) => match (self.evaluate_inner(l), self.evaluate_inner(r)) {
                (ExecResult::Value(RelType::Int(a)), ExecResult::Value(RelType::Int(b))) => {
                    ExecResult::Value(RelType::Int(a >> b))
                }
                _ => ExecResult::Fault {
                    msg: "Bitwise SHR expects integers".into(),
                    node: "Node::BitShiftRight".into(),
                },
            },

            // Control Flow
            Node::If(cond, then_b, else_b) => match self.evaluate_inner(cond) {
                ...
                _ => ExecResult::Fault {
                    msg: "If condition must be boolean".into(),
                    node: "Node::If".into(),
                },
            },
            ...
            Node::Return(expr) => {
                let v = match self.evaluate_inner(expr) {
                    ExecResult::Value(v) => v,
                    err => return err,
                };
                ExecResult::ReturnBlockInfo(v)
            ...
            Node::FnDef(name, params, body) => {
                self.set_var(
                    name.clone(),
                    RelType::FnDef(name.clone(), params.clone(), body.clone()),
                );
                ExecResult::Value(RelType::Void)
            }
            Node::Call(name, args) => {
                let func = if let Some(f) = self.get_var(name) {
                    f
                } else {
                    return ExecResult::Fault {
            ...
            Node::CheckCollision {
                ...
                let am = match self.evaluate_inner(a_min) {
                    ExecResult::Value(v) => v,
                    err => return err,
                };
                let ax = match self.evaluate_inner(a_max) {
                    ExecResult::Value(v) => v,
                    err => return err,
                };
                let bm = match self.evaluate_inner(b_min) {
                    ExecResult::Value(v) => v,
                    err => return err,
                };
                let bx = match self.evaluate_inner(b_max) {
                    ExecResult::Value(v) => v,
                    err => return err,
                };
                let v_am = if let Some(v) = self.to_vec3(am) {
                    v
                } else {
                    return ExecResult::Fault {
                        msg: "a_min must be array".into(),
                        node: "Node::CheckCollision".into(),
                    };
                };
                let v_ax = if let Some(v) = self.to_vec3(ax) {
                    v
                } else {
                    return ExecResult::Fault {
                        msg: "a_max must be array".into(),
                        node: "Node::CheckCollision".into(),
                    };
                };
                let v_bm = if let Some(v) = self.to_vec3(bm) {
                    v
                } else {
                    return ExecResult::Fault {
                        msg: "b_min must be array".into(),
                        node: "Node::CheckCollision".into(),
                    };
                };
                let v_bx = if let Some(v) = self.to_vec3(bx) {
                    v
                } else {
                    return ExecResult::Fault {
                        msg: "b_max must be array".into(),
                        node: "Node::CheckCollision".into(),
                    };
                };
            ...
            // Delegation
            Node::ToString(expr) => match self.evaluate_inner(expr) {
            ...
            | Node::MouseGrab { .. }
            | Node::RaycastSimple
            | Node::WeaponViewModel { .. }
            | Node::Fetch { .. }
            | Node::Extract { .. }
            | Node::EvalJSONNative(_)
            | Node::NativeCall(_, _)
            | Node::ExternCall { .. }
            ...
            | Node::LoadMesh(_)
            | Node::LoadTexture(_)
            | Node::RenderAsset(_, _, _, _)
            | Node::LoadFont(_)
            | Node::DrawText(_, _, _, _, _)
            | Node::GetLastKeypress
            | Node::UIWindow(_, _, _)
            | Node::UILabel(_)
            | Node::UIButton(_)
            | Node::UITextInput(_)
            | Node::UISetStyle(_, _, _, _, _, _)
            | Node::UIHBox(_)
            | Node::UIVBox(_)
            | Node::UIHorizontal(_)
            | Node::UIFullscreen(_)
    ...
    pub fn do_math(&mut self, left: &Node, op: char, right: &Node) -> ExecResult {
        let lv = match self.evaluate_inner(left) {
            ExecResult::Value(v) => v,
            err => return err,
        };
        let rv = match self.evaluate_inner(right) {
            ExecResult::Value(v) => v,
            err => return err,
        };
        let res = match op {
    ...
    pub fn do_compare(&mut self, left: &Node, op: &str, right: &Node) -> ExecResult {
        ...
        let res = match op {
            ...
            _ => {
                return ExecResult::Fault {
                    msg: format!("Unknown comparison: {}", op),
                    node: "Unknown".into(),
                };
            }
        };
