...
fn test_yield_opcode_suspends_and_resumes() {
    ...
    assert!(
        vm.is_yielded(),
        "VM must be in Yielded state after OpCode::Yield"
    );
    ...
    let res2 = vm
        .resume(&instructions, &constants, &sandbox_perms(), None)
        .unwrap();
    assert_eq!(
        res2,
        RelType::Int(150),
        "Resumed execution must compute 100 + 50 = 150"
    );
    assert_eq!(
        *vm.execution_state(),
        VmExecutionState::Finished(RelType::Int(150))
    );
}
...
fn test_multiple_sequential_yields() {
    ...
    // Yield 2
    vm.resume(&instructions, &constants, &sandbox_perms(), None)
        .unwrap();
    assert!(vm.is_yielded());
...
fn test_resume_non_yielded_vm_returns_error() {
    ...
    assert_eq!(
        *vm.execution_state(),
        VmExecutionState::Finished(RelType::Int(42))
    );
