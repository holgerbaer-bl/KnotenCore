    ...
    fn handle(
        ...
        match func_name {
            ...
            "Math.Sin" => {
                if args.len() != 1 {
                    return Some(ExecResult::Fault {
                        msg: "Math.Sin expects 1 argument".to_string(),
                        node: "Native::Math.Sin".into(),
                    });
                }
                match args[0] {
                    RelType::Float(f) => Some(ExecResult::Value(RelType::Float(f.sin()))),
                    RelType::Int(i) => Some(ExecResult::Value(RelType::Float((i as f64).sin()))),
                    _ => Some(ExecResult::Fault {
                        msg: "Math.Sin expects a Number".to_string(),
                        node: "Native::Math.Sin".into(),
                    }),
                }
            }
            "Math.Cos" => {
                if args.len() != 1 {
                    return Some(ExecResult::Fault {
                        msg: "Math.Cos expects 1 argument".to_string(),
                        node: "Native::Math.Cos".into(),
                    });
                }
                match args[0] {
                    ...
                    _ => Some(ExecResult::Fault {
                        msg: "Math.Cos expects a Number".to_string(),
                        node: "Native::Math.Cos".into(),
                    }),
                }
            }
            "Math.Floor" => {
                ...
                match args[0] {
                    ...
                    _ => Some(ExecResult::Fault {
                        msg: "Math.Floor expects a Number".to_string(),
                        node: "Native::Math.Floor".into(),
                    }),
                }
            }
            "Math.Ceil" => {
                ...
                match args[0] {
                    ...
                    RelType::Int(i) => Some(ExecResult::Value(RelType::Int(i))),
                    _ => Some(ExecResult::Fault {
                        msg: "Math.Ceil expects a Number".to_string(),
                        node: "Native::Math.Ceil".into(),
                    }),
                }
            }
